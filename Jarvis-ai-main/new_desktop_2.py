...
class JarvisOverlayGUI(QWidget):
    ...
    def setupUi(self):
        ...
        self.TerminalText.setStyleSheet("""
            ...
            background-color: transparent;
        """)
        self.TerminalText.setVerticalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOn)
        self.TerminalText.setHorizontalScrollBarPolicy(Qt.ScrollBarPolicy.ScrollBarAlwaysOff)
    ...
    def update_gif_state(self, state):
        """
        Update which GIF is displayed based on the current state.
        Valid states: 'speaking', 'listening', 'loading', 'sleeping'
        """
        # Hide all GIFs first
        ...
        # Show only the appropriate GIF based on the state
        if state == "speaking":
            self.speaking_gif.show()
            self.TerminalText.appendPlainText("NOVA is speaking...")
        elif state == "listening":
            self.listening_gif.show()
            self.TerminalText.appendPlainText("NOVA is listening...")
        elif state == "loading":
            self.loading_gif.show()
            self.TerminalText.appendPlainText("NOVA is processing...")
        elif state == "sleeping":
            self.sleeping_gif.show()
            self.TerminalText.appendPlainText("NOVA is in sleep mode...")
        else:
            # Default to sleeping state if an invalid state is provided
            self.sleeping_gif.show()
