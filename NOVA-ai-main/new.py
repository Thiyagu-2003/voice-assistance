...
from api import speak, listen, process_command, search_pincode, search_book, get_random_recipe, get_lyrics, get_quote, get_fun_fact, call_anime_quote_api, call_anime_character_api, get_word_definition

# Global variables
active = True  # Global active state
window = None  # Global window reference


# Store window reference as a mutable container
...
def safe_terminal_print(text):
    """Safely print to terminal with global window check"""
    global window
    try:
        if window and hasattr(window, 'append_to_terminal'):
            window.append_to_terminal(str(text))  # Use GUI terminal output
        else:
            print(text)  # Fallback to standard terminal
    except Exception as e:
        print(f"Terminal print error: {e}")
...
def safe_terminal_print(text):
    ...
    global window
    try:
        if window and hasattr(window, 'terminaltext'):
            window.terminaltext(str(text))
    except Exception as e:
        print(f"Terminal print error: {e}")
        print(text)  # Fallback to standard print
...
def update_assistant_state(state):
    ...
    if window:
        try:
            ...
            safe_terminal_print(f"State update error: {e}")

def speak(audio):
    """Speak the given text and update assistant state."""
    try:
        update_assistant_state("speaking")
        engine.say(audio)
        engine.runAndWait()
        update_assistant_state("sleeping")
    except Exception as e:
        safe_terminal_print(f"Speech error: {e}")
...
def listen_command():
    ...
    try:
        ...
        with sr.Microphone() as source:
            safe_terminal_print("Listening...")
            recognizer.adjust_for_ambient_noise(source)
            audio = recognizer.listen(source, timeout=4)
        
        update_assistant_state("loading")
        query = recognizer.recognize_google(audio, language='en-in').lower()
        safe_terminal_print(f"You said: {query}\n")
        return query
        ...
        safe_terminal_print(f"Listening error: {e}")
        return ""
...
# Existing functions remain the same, replacing window.terminaltext() with safe_terminal_print()
...
class MainThread(QtCore.QThread):
    ...
    def takeCommand(self):
        try:
            ...
            safe_terminal_print("Recognizing...")
            query = r.recognize_google(audio, language='en-in')
            safe_terminal_print(f"User said: {query}\n")
            return query.lower()
        except Exception as e:
            safe_terminal_print("Say that again please...")
            return "None"
...
class JarvisApp:
    ...
    def start(self):
        ...
        try:
            ...
            safe_terminal_print("Launching Desktop 2...")
            ...
            safe_terminal_print("Starting Main Task Execution...")
            self.core.start()
            
            return self.core
            
        except Exception as e:
            safe_terminal_print(f"Error in Jarvis execution: {e}")
            sys.exit(1)
...
if __name__ == "__main__":
    try:
        ...
        window_ref.set(JarvisOverlayGUI())  # Set the GUI instance
        window = window_ref.get()  # Ensure global window is linked

        sys.exit(app.exec())

    except Exception as e:
        safe_terminal_print(f"Error in main execution: {e}")
        sys.exit(1)